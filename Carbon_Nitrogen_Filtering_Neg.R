...
carbon_metabolites <- meta_neg_clean %>%
  filter(
...
nitrogen_metabolites <- meta_neg_clean %>%
  filter(
...
cn_neg_all <- bind_rows(carbon_metabolites, nitrogen_metabolites) %>%
  distinct(Name, .keep_all = TRUE)

# ===================== 4. Export =====================
cat("Carbon Metabolites Count:", nrow(carbon_metabolites), "\n")
cat("Nitrogen Metabolites Count:", nrow(nitrogen_metabolites), "\n")
cat("Total Unique C/N Metabolites:", nrow(cn_neg_all), "\n")

write.csv(cn_neg_all, "XXXX.csv", row.names = FALSE, fileEncoding = "UTF-8")
