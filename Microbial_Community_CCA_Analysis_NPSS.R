...
perform_cca_publish <- function(community_data, soil_data, title, genus_column = NULL, 
  ...
  if (!is.null(cor_matrix) && ncol(cor_matrix) > 1) {
    ...
    if (length(high_cor) > 0) {
      soil_data <- soil_data[, -high_cor, drop = FALSE]
    }
  }
  ...
  p <- ggplot() +
    geom_point(data = as.data.frame(cca_scores$sites), 
               aes(x = CCA1, y = CCA2), 
               size = 3.5, color = "#d00000", alpha = 0.8, shape = 21, 
               fill = alpha("#d00000", 0.6)) +
    geom_segment(data = as.data.frame(cca_scores$biplot), 
                 aes(x = 0, y = 0, xend = CCA1 * env_scale, yend = CCA2 * env_scale), 
                 arrow = arrow(length = unit(0.25, "cm"), type = "closed"), 
                 color = "#E09D94", linewidth = 0.8) +
    geom_text(data = as.data.frame(cca_scores$biplot), 
              aes(x = CCA1 * env_scale * 1.1, y = CCA2 * env_scale * 1.1, label = rownames(cca_scores$biplot)), 
              color = "#E09D94", size = 4.5, fontface = "bold")
  ...
  if (!is.null(genus_column)) {
    ...
    if (length(top_species) > 0) {
      ...
      p <- p +
        geom_point(data = species_data, aes(CCA1, CCA2), 
                   size = 4, color = "#FFB55F", shape = 17, stroke = 1.2) +
        geom_text_repel(data = species_data, aes(CCA1, CCA2, label = Genus),
                        color = "#FFB55F", size = label_size + 0.5, fontface = "italic",
                        box.padding = unit(label_distance, "lines"),
                        max.overlaps = max_overlaps)
    }
  }
  
  title_text <- paste0(title, " CCA Analysis\n",
                       "Adj R² = ", variance_explained_adj, "%, Global P ", p_results$p_global_formatted)
  
  p <- p + labs(title = title_text, x = paste0("CCA1 (", cca1_var, "%)"), y = paste0("CCA2 (", cca2_var, "%)")) +
    theme_bw() +
...
process_microbial_data_publish <- function(microbial_data, soil_data, title) {
  ...
  cca_result <- perform_cca_publish(data_filtered, soil_filtered, title, genus, 
                                    label_distance = 0.8, label_size = 3.5, max_overlaps = 30)
