...
# ===================== 2. 原生生物数据预处理 =====================
otu_table <- read.csv("原生生物NPSS.csv", row.names = 1, sep = ",", fileEncoding = "UTF-8")
taxonomy_table <- read.csv("原生生物注释表.csv", row.names = 1, sep = ",", fileEncoding = "UTF-8")
...
t1$res_node_table$group_type <- sapply(t1$res_node_table$module, function(m) {
  pos <- sum(mod_stats$count[mod_stats$mod_s == m & mod_stats$corr_type == "positive"])
  neg <- sum(mod_stats$count[mod_stats$mod_s == m & mod_stats$corr_type == "negative"])
  ifelse(pos >= neg, "Group_Pos", "Group_Neg")
})

# ===================== 输出文件 =====================
t1$save_network(filepath = "NPSS原生生物_network.gexf")
write.csv(t1$res_node_table, "Gephi_节点表_NPSS原生生物.csv", row.names = FALSE)
write.csv(t1$res_edge_table, "Gephi_边表_NPSS原生生物.csv", row.names = FALSE)

cat("\nGephi处理提示：\n1. 导入gexf，使用ForceAtlas 2布局。\n2. 节点颜色：使用Partition，选择group_type。\n3. 边颜色：使用Partition，选择corr_type。\n")
