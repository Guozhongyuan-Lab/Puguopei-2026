...
wd="F:/iCAMP_NPSS+PSS/Fungi"

# the OTU table file (Tab delimited txt file)
com.file<- read.csv("F:/iCAMP_NPSS+PSS/Fungi/Fungi.csv", header = TRUE, row.names = 1, 
...
tree.file="Fungi_tree.nwk"

# the classification (taxonomy) information
clas.file="Fungi_classification.csv"
...
save.wd="F:/iCAMP_NPSS+PSS/Fungi/Fungi_iCAMP_ analysis"
...
otu <- read.csv("F:/iCAMP_NPSS+PSS/Fungi/Fungi.csv", header = TRUE, row.names = 1,  stringsAsFactors = FALSE)
taxa <- read.csv("F:/iCAMP_NPSS+PSS/Fungi/Fungi_classification.csv", header = TRUE, row.names = 1,  stringsAsFactors = FALSE)
metadata <- read.csv("F:/iCAMP_NPSS+PSS/Fungi/sample_groups.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
tree <- read.tree("F:/iCAMP_NPSS+PSS/Fungi/Fungi_tree.nwk")
...
# 读取bins信息
bins <- read.csv("F:/iCAMP_NPSS+PSS/Fungi/Fungi_iCAMP_ analysis/Test.Bin_TopTaxon.csv", row.names = 1, header = TRUE)
bins_assembly <- read.csv("F:/iCAMP_NPSS+PSS/Fungi/Fungi_iCAMP_ analysis/Test.ProcessImportance_EachBin_EachGroup.csv", header = TRUE)
...
# 导入bins的相对丰度
bins_RA <- read.csv("F:/iCAMP_NPSS+PSS/Fungi/Fungi_iCAMP_ analysis/Test.Bin_TopTaxon.csv", row.names = 1, header = TRUE)
...
p21 <- ggtree(treeWithInfo,
              ...
              open.angle = 10,
              lwd = 1) +
  geom_tiplab(aes(color = group), offset = 14, size = 1.5, show.legend = FALSE) +
...
p23
ggsave("F:/iCAMP_NPSS+PSS/Fungi/Fungi_icamp_radial_tree.pdf", plot = p23, dpi = 1200, width = 8, height = 8)
