...
wd="F:/iCAMP_NPSS+PSS/Protists"

# the OTU table file (Tab delimited txt file)
com.file<- read.csv("F:/iCAMP_NPSS+PSS/Protists/Protists.csv", header = TRUE, row.names = 1, 
...
tree.file="Protists_tree.nwk"

# the classification (taxonomy) information
clas.file="Protists_classification.csv"
...
save.wd="F:/iCAMP_NPSS+PSS/Protists/Protists_iCAMP_ analysis"
...
# 查看bin的数量和每个bin中包含的OTU数量
bin_summary <- table(phylobin$sp.bin[, 3])
print(bin_summary)
cat("\n总bin数:", length(bin_summary))
...
otu <- read.csv("F:/iCAMP_NPSS+PSS/Protists/Protists.csv", header = TRUE, row.names = 1,  stringsAsFactors = FALSE)
taxa <- read.csv("F:/iCAMP_NPSS+PSS/Protists/Protists_classification.csv", header = TRUE, row.names = 1,  stringsAsFactors = FALSE)
metadata <- read.csv("F:/iCAMP_NPSS+PSS/Protists/sample_groups.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
tree <- read.tree("F:/iCAMP_NPSS+PSS/Protists/Protists_tree.nwk")
...
# 读取bins信息
bins <- read.csv("F:/iCAMP_NPSS+PSS/Protists/Protists_iCAMP_ analysis（0.2，5）/Test.Bin_TopTaxon.csv", row.names = 1, header = TRUE)
bins_assembly <- read.csv("F:/iCAMP_NPSS+PSS/Protists/Protists_iCAMP_ analysis（0.2，5）/Test.ProcessImportance_EachBin_EachGroup.csv", header = TRUE)
...
# 导入bins的相对丰度
bins_RA <- read.csv("F:/iCAMP_NPSS+PSS/Protists/Protists_iCAMP_ analysis（0.2，5）/Test.Bin_TopTaxon.csv", row.names = 1, header = TRUE)
...
p21 <- ggtree(treeWithInfo,
              ...
              open.angle = 10,
              lwd = 0.5) +
  geom_tiplab(aes(color = group), offset = 16.5, size = 1, show.legend = FALSE) +
...
p23
ggsave("F:/iCAMP_NPSS+PSS/Protists/Protists_icamp_radial_tree（0.2，5）.pdf", plot = p23, dpi = 1200, width = 8, height = 8)
